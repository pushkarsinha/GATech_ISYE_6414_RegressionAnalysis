...
library(ggplot2)
ggplot(treatement_shift_data, aes(x=Treatment, y=Phase_Shift_hr), xlab = "Treatement", ylab = "Phase Shift (hr)") + geom_boxplot() + theme(axis.text.x  = element_text(angle=90, vjust=0.5))+ggtitle("Phase Shift (hr) with Respect to Treatement - ANOVA")
...
```{r}
# Your code here...
plot(data$performance, data$chmax, main="Scatterplot of CPU performance and the maximum number of channels", xlab="CPU performance",ylab="maximum number of channels")
...
**Comment** : correlation cofficient of 0.6052093 shows that the correlateion is above Modereate and not strong positive relationship
...
a. **3 pts** What are the model parameters and what are their estimates?  
   **Answer:**  
   Model parameters 
     i. $\beta_0$ - Intercept - 37.2252
     ii. $\beta_1$ - Max #Channels - 3.7441
     iii. $\hat{\sigma}$ - Std Error- 0.3423
...
e. **2 pts** Is $\beta_1$ statistically significantly positive at an $\alpha$-level of 0.01?  What is the approximate p-value of this test?
   ...
   **Answer:**  
   p-value is 1.424772e-22 for lower tail.. This shows that  $\beta_1$ statistically significantly positive at an $\alpha$-level of 0.01
...
```{r}
# Your code here...
plot(log(data$performance), log(data$chmax + 1), main="Linearity Assumption", xlab="CPU performance",ylab="maximum number of channels")
```
...
```
    ...
    **Answer : ** 
    1. Model 1 - the predicted value - 516.4685 with confidence interval of 252.2519 & 780.6851
      1. the predicted value is high compared to model 2 but has smaller range.
    2. Model 2 - the predicted value - 277.723 with confidence interval of 55.17907 & 1397.813
      1. the predicted value is lower than Model 1 and it has higher prediction intervel. 
...
```{r}
# Your code here...
ggplot(data2, aes(x=vendor, y=performance), xlab = "Vendor", ylab = "Performance") + geom_boxplot() + theme(axis.text.x  = element_text(angle=90, vjust=0.5))+ggtitle("Performance with Respect to Vendor")
```
   **Answer : ** 
    1. honeywell and hp look to have similar meanst
    2. nas seems to have higher mean that honeywell and hp
    3. eventhough honewell has lower mean, there are some higher performance machines that they make
